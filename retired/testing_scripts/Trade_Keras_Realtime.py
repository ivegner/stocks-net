...
builder = build_realtime_data(
    test_secs, start_date="2014-01-01", end_date="2016-01-01", backload=200
)

backload_data = builder.send(
    None
)  # OrderedDict [(date, [{data: data1, price:price1}, {data: data2, price:price2}]), (date2, ...)]
...
print(
    ...
    round((cash - TEST_CASH) / (day_count / trading_days_in_year)),
    "\t\t|",
)
print(
    "|  Annualized return percent:",
    round(((cash / TEST_CASH) * 100) / (day_count / trading_days_in_year)) - 100,
    "\t\t|",
)
