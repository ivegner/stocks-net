...
_build_data = build_data(random_split = False, start_date = "2003-01-01", end_date = "2017-01-01", raw = True)
	...
	while(1):	# every price-tick
		if bought_flag == 1:
			if action == 0:   
				...
				price, action = yield potential_profit_per_share # reward = potential profit if sold
			else:
...
for i in range(epochs):
	...
	for sec_idx in range(len(train_secs)):
		...
		while state is not None and status == 1:
			...
			x = state["x"]
			x = np.reshape(x, (1, one_input_length)) # state is a day of shape (1, one_input_length?)
			price = state["price"]
			# print(state)
			#We are in state S
			...
			if (len(replay) < buffer): #if buffer not filled, add to it
				...
				for memory in minibatch:
					...
					old_qval = model.predict(np.reshape(old_state["x"], (1,one_input_length)), batch_size=1)
					newQ = model.predict(np.reshape(new_state["x"], (1,one_input_length)), batch_size=1)
					maxQ = np.max(newQ)
					y = np.zeros((2,))
					y[:] = old_qval[:]
