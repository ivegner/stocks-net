...
for epoch in range(initial_epoch, n_epochs):
	...
	for sec in Bar("Processing", suffix = "%(percent)d%%").iter(sys.argv[(2 if load_model_arg else 1):]):	#built-in progress bar that also iterates
	...
	print("Accuracy:",accuracy.eval({x:builder.send("WIKI/AAPL")["testX"], y:builder.send("WIKI/AAPL")["testY"]}, session = sess))
