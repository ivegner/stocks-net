...
aapl = data_builder.build_data(["WIKI/AAPL"], start_date = "2014-01-01", 
						   					  end_date = "2017-01-01")[0]
...
if not load_model_arg:
	model = Sequential()
	model.add(GRU(1000, input_dim = one_input_length, return_sequences = True, W_regularizer = l2(0.1), U_regularizer = l2(0.1)))
	model.add(GRU(1000, return_sequences = True, W_regularizer = l2(0.1), U_regularizer = l2(0.1)))
	model.add(GRU(1000, return_sequences = True, W_regularizer = l2(0.1), U_regularizer = l2(0.1)))
	# model.add(GRU(500, return_sequences = True, W_regularizer = l2(0.1), U_regularizer = l2(0.1)))
	# model.add(GRU(500, return_sequences = True, W_regularizer = l2(0.1), U_regularizer = l2(0.1)))
...
model.fit_generator(keras_builder(), samples_per_epoch = samples_per_epoch, nb_epoch=nb_epoch, 
					verbose = 2, callbacks=[
